...
public class LotOrdersPage {
	
	@FindBy(xpath="//span[text()=' आर्डर आईडी :']/..//span[2]")
	...
	public LotOrdersPage(WebDriver driver) {
	...
	public void selectOrderIdForOrderTackingTab() throws IOException {
	...
	public void validateOrderTrackingOrderIdFRomGlobalScanner() {
		...
		String id = ExcellUtilites.readData("sheet1", 1, 12);
		orderIdPlaceHoldrBtn.sendKeys(id);
		proceedKareBtn.click();
		WaitStatementLib.iSleep(1);
		String actResult = actMsg.getText();
		String expResult = "आर्डर ट्रैकिंग पर जाएँ";
		AssertJUnit.assertEquals(expResult, actResult);
		Reporter.log(infoMsg.getText(),true);
		Reporter.log("OrderTaking  ORDER SCAN SUCCESSFULLY THROUGH GLOBAL SCANNER  ",true);	
		crossBtn.click();
